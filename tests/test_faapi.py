...
def test_user(cookies: RequestsCookieJar, user_test_data: dict):
    ...
    assert compare_dates(user.join_date, user_dict["join_date"], 1)
    assert compare_dates(user.join_date, datetime.fromisoformat(user_test_data["join_date"]), 1)
...
def test_journal(cookies: RequestsCookieJar, journal_test_data: dict):
    ...
    assert compare_dates(journal.date, journal_dict["date"], 1)
    assert compare_dates(journal.date, datetime.fromisoformat(journal_test_data["date"]), 1)
